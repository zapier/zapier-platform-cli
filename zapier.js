...
var buildCmd = function() {
  ...
  return ensureDir(tmpDir)
    ...
    .then(function() {
      ...
      return makeZip(tmpDir, wdir + '/build.zip');
    })
    .then(function() {
      printDone();
...
buildCmd.docs = 'Builds a deployable zip from the current directory. (Debug only)';
...
var pushCmd = function() {
  return checkCredentials()
    .then(getCurrentApp)
    .then(function(app) {
      var definition = readFile('app-definition.json')
        .then(function(buf) {
          return JSON.parse(buf.toString());
        });
      var zipFile = readFile('build.zip')
        .then(function(buf) {
    ...
    .then(function(values) {
      ...
      var app = values[2];
      console.log('Preparing to push version ' + definition.version + ' of your app "' + app.title + '".\n');

      printStarting('  Pushing version ' + definition.version);
...
pushCmd.docs = 'Push a new version of the current app - does not deploy.';
pushCmd.example = 'zapier push 1.0.0';
